...
const protect = async (req, res, next) => {
    try {
        ...
        const decoded = await jwtVerifyPromisified(token, process.env.JWT_SECRET || "my-pos-indentifier");
...
const login = async (req, res) => {
    try {
        ...
        const secretKey = process.env.JWT_SECRET || "my-todos-indentifier";
...
const register = async (req, res) => {
    try {
        ...
        const secretKey = process.env.JWT_SECRET || "my-pos-indentifier";
        const expires = process.env.JWT_EXPIRES_IN || "30d";
        const token = jsonwebtoken_1.default.sign({ id: user.id }, secretKey, {
            expiresIn: expires
        });
